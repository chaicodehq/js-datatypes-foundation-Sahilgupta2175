...
export function writePostcard(sender, receiver, message) {
  ...
  let postMessage = `Priy ${receiver},\n\n${message}\n\nAapka/Aapki,\n${sender}`;

  return postMessage;
}

export function isValidPincode(code) {
  if (typeof (code) !== "string" || code.length !== 6 || code.startsWith('0') || !code.match(/^\d+$/)) {
    return false;
  }
...
export function formatPostcardField(label, value, width) {
  ...
  if (width > 0) {
    formatPostCard = `${label.padEnd(width, " ")}: ${value}`;
  } else {
    formatPostCard = `${label.padEnd(12, " ")}: ${value}`;
  }

  return formatPostCard;
}
...
export function countVowels(message) {
  ...
  let matchVowels = message.match(/[aeiouAEIOU]/g);

  return matchVowels === null ? 0 : matchVowels.length;
}
